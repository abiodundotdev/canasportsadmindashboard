...
export default function Dashbaord(props){
    ...
    const handleFormSubmit = () => {
        ...
        User.saveDataToServer(formJson,"/savecupname").then(
            (res) => {
                toast.success("New Cup Added Successfully")
        ...
        useEffect(
            () => {
               User.getServerData("/getcuplist").then(
                   (response) => {
              ...
              .then(willDelete => {
                if (willDelete) {
                    User.saveDataToServer({'id' : id}, "/deleteleague").then(
                        (response) =>  {
                            toast.success(name+" Cup Deleted Successfully")
        ...
        <div className="col-lg-6">
            <div class="card">
                <div className="card-header">
                    <h3 className="card-title text-center">Add New Cup</h3>
                </div>
        <div className="card-body">
            <div className="">
                <div className="form-group">
                    <label for="formGroupExampleInput">Cup Name</label>
                ...
                <div className="form-group">
                    <label for="formGroupExampleInput">Cup Image</label>
                    <input type="file" onChange={(e) => setImage(e.target.value)} className="form-control" id="descrption" />
                </div>
                ...
                <button onClick={handleFormSubmit} className="btn btn-success mt-10 text-center d-flex" disabled={isFormDiabled}><i className="mr-2"><RiSaveFill /></i> <small className="mr-2">ADD NEW CUP</small> {preloader}  </button>
            </div>
...

{
    !pageReady ? <LoadingSkeleton length={10}/> :  <div className="card">
               <div className="card-header">
                   <h6 className="card-title">Cups</h6>
               </div>
                    ...
                    <tbody>
                      {
                          fetchedData?.map(
                              (eachdata) => {
                                  return <tr> <td>{eachdata.cup_name}</td> <td>{eachdata.team_number}</td> <td>{eachdata.start_date}</td> <td>{eachdata.end_date}</td>  <td>{eachdata.cup_id}</td> <td style={{cursor : "pointer"}} onClick={()=>deleteLeague(eachdata.cup_name,eachdata.id)}><i><FaTrash /></i></td> </tr>
                              }
                          )
                      }
                    </tbody>
