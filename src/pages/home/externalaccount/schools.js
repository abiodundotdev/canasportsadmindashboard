...
export default function Schools(props){
    ...
    let number_of_page = Math.ceil(fetchedData?.length / showPerPage)
      ...
      const filteredusers = fetchedData?.filter( (eachdata) => 
         eachdata.sport_dir_name.includes(keyword) || eachdata.school_name.includes(keyword)
      ) 
    ...
    const handleApproval = (type,action,detail) => {
      ...
      swal({
        ...
        .then(willDelete => {
          if (willDelete) {
              User.saveDataToServer(formData, "/verifyschool").then(
                  (response) =>  {
    ...
    useEffect(
        () => {
            User.getServerData("/getallschools/all").then(
                (response)=>{
...
<thead>
    <tr>
    <th></th>
    <th scope="col"><small>School Name /Sport Director</small></th>
    <th scope="col"><small>Address</small></th>
    <th scope="col"><small>Head of School</small></th>
...
<tbody>
   {
    userstoshow?.map( (eachuser)=> {
        return (
            ...
            <td>
                <div><a href="javascript:void(0);">{eachuser?.school_name}</a></div>
                <div className="text-muted">{eachuser?.sport_dir_name}</div>
            </td>
            ...
            <td className="hidden-sm">
                <div className="text-muted">{eachuser?.head_of_school}</div>                                                
            </td>
        ...
        />
            ...
            <Modal show={smodal}  onHide={() => true}>
                    ...
                    <Modal.Body>
                        ...
                        <tr>
                            <td className="text-success">School Logo</td>
